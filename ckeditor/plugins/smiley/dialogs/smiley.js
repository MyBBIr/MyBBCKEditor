...
CKEDITOR.dialog.add( 'smiley', function( editor ) {
	...
	var html = [
		'<div style="overflow:auto;max-width:300;max-height:400px;">' +
	...
	for ( i = 0; i < size; i++ ) {
		...
		html.push( '<td class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation">' +
			'<a href="javascript:void(0)" role="option"', ' aria-posinset="' + ( i + 1 ) + '"', ' aria-setsize="' + size + '"', ' aria-labelledby="' + smileyLabelId + '"', ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">', '<img class="cke_hand" title="', config.smiley_descriptions[ i ], '"' +
					' cke_src="', CKEDITOR.tools.htmlEncode( images[ i ] ), '" alt="', config.smiley_descriptions[ i ], '"', ' src="', CKEDITOR.tools.htmlEncode( images[ i ] ), '"',
	...
	return {
		...
		buttons: [ CKEDITOR.dialog.cancelButton, mybbmore.Button ],
		onOk: function() {
			var dialog = this;
			window.open('misc.php?action=smilies&popup=true&editor=clickableEditor', 'smilies', 'resizable=yes,status=no,location=no,toolbar=no,menubar=no,fullscreen=no,scrollbars=yes,dependent=no,width=300,left=80,height=450,top=50');
		}
	};
});
