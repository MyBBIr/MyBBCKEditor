...
(function () {
	...
	CKEDITOR.htmltobbcode = function(a) {
		if(a != '')
		{
			postData = "m="+encodeURIComponent(a).replace(/\+/g, "%2B");
			var responseXML = CKEDITOR.mybbajax.open({
					url: CKEDITOR.basePath+'ckeditor.php?action=htmlparser',
					type: 'POST',
					data: postData
			});
			a =responseXML.responseText;
		}
		return a;
	}
	
	CKEDITOR.bbcodetohtml = function(a) {
		if(a != '')
		{
			postData = "m="+encodeURIComponent(a).replace(/\+/g, "%2B");
			var responseXML = CKEDITOR.mybbajax.open({
					url: CKEDITOR.basePath+'ckeditor.php?action=bbcodeparser',
					type: 'POST',
					data: postData
			});
			a =responseXML.responseText;
	...
	(function () {
		CKEDITOR.inline = function (b, c) {
			...
			b = CKEDITOR.dom.element.getCKEDITOR.dom.element.get(b);
	...
	(function () {
		...
		function a(a) {
			...
			b = CKEDITOR.dom.element.createFromHtml(e.output({
				...
				bottomHtml: l ? '<span id="' + a.ui.spaceId("bottom") + '" class="cke_bottom cke_reset_all" role="presentation"><span id="autosave"></span>' + l + "</span>" : "",
	...
	(function () {
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				getData: function (a) {
					var b = this.getHtml();
					!a && this.editor.dataProcessor && (b = CKEDITOR.htmltobbcode(this.editor.dataProcessor.toDataFormat(b)));
					return b
				},
				setReadOnly: function (a) {
...
*/
	...
	CKEDITOR.plugins.add("customstylesheet", {
		onLoad: function() {
			var stylesheet = document.createElement('link');
			stylesheet.setAttribute('rel', 'stylesheet');
			stylesheet.setAttribute('type', 'text/css');
			stylesheet.setAttribute('id', 'mybbcss');
			stylesheet.setAttribute('href', CKEDITOR.basePath+'stylesheet.php');
	...
	CKEDITOR.plugins.add( 'videos', {
		...
		init: function( editor ) {
			editor.addCommand( 'videosDialog', new CKEDITOR.dialogCommand( 'videosDialog' ) );
			...
			CKEDITOR.dialog.add( 'videosDialog', function ( editor ) {
				return {
					...
					onOk: function() {
						var dialog = this;
						editor.insertText( '[video='+dialog.getValueOf( 'tab-basic', 'video' )+']'+dialog.getValueOf( 'tab-basic', 'videourl' )+'[/video]' );
					}
				};
			});
		}
	...
	(function () {
		...
		CKEDITOR.plugins.add("font", {
			...
			init: function (a) {
				...
				g(a, "FontSize", "size", a.lang.font.fontSize, b.fontSize_sizes, b.fontSize_defaultLabel, b.fontSize_style, 30)
			}
		})
	})();
	CKEDITOR.config.font_names = "Arial/Arial;Courier/Courier;Impact/Impact;Tahoma/Tahoma;Times New Roman/Times New Roman;Trebuchet MS/Trebuchet MS;Verdana/Verdana";
			...
			init: function (a) {
				...
				a.addMode("wysiwyg", function (d) {
					...
					var c = "document.open();" + (CKEDITOR.env.ie ? "(" + CKEDITOR.tools.fixDomain + ")();" : "") + "document.close();",
						...
						e = CKEDITOR.dom.element.createFromHtml('<iframe src="' + c + '" frameBorder="0" contenteditable="true"></iframe>');
					e.setStyles({
						width: "100%",
						height: "100%"
					});
		...
		var j = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				getData: function (a) {
					...
					a.dataProcessor && (b = CKEDITOR.htmltobbcode(a.dataProcessor.toDataFormat(b)));
					e && (b = e + "\n" + b);
					c && (b = c + "\n" + b);
					return b
				},
	...
	CKEDITOR.plugins.add("smiley", {
		requires: "dialog",
		init: function (a) {
			a.config.smiley_path = a.config.smiley_path || '';
	...
	CKEDITOR.config.smiley_images = "".split(" ");
	CKEDITOR.config.smiley_descriptions = "".split(";");
	(function () {
		function l(a, c) {
			var c = void 0 === c || c,
				b;
	...
	CKEDITOR.unlinkCommand.prototype = {
		...
		refresh: function (b, a) {
			...
			d && "a" == d.getName() && d.getAttribute("href") && d.getChildCount() ? this.setState(CKEDITOR.TRISTATE_OFF) : this.setState(CKEDITOR.TRISTATE_OFF)
		},
		contextSensitive: 1,
		startDisabled: 0,
		requiredContent: "a[href]"
	...
	var bbcodeMap = { b: 'strong', u: 'u', sub: 'sub', sup: 'sup', i: 'em', s: 's', color: 'span', size: 'span', bgcolor: 'span', font: 'span', align: 'div', dir: 'div', quote: 'blockquote', code: 'code', php: 'pre', url: 'a', email: 'span', img: 'span', '*': 'li', list: 'ol' },
		convertMap = { strong: 'b', b: 'b', strike: 's', s: 's', u: 'u', sub: 'sub', sup: 'sup', em: 'i', i: 'i', code: 'code', pre: 'php', li: '*' },
		tagnameMap = { strong: 'b', strike: 's', em: 'i', u: 'u', sub: 'sub', sup: 'sup', li: '*', ul: 'list', ol: 'list', code: 'code', pre: 'php', a: 'link', img: 'img', blockquote: 'quote' },
		stylesMap = { color: 'color', bgcolor: 'background-color', size: 'font-size', font: 'font-family', align: 'text-align', dir: 'direction' },
		attributesMap = { url: 'href', email: 'mailhref', quote: 'cite', list: 'listType', dir: 'dir' };
	...
	CKEDITOR.BBCodeParser.prototype = {
		parse: function( bbcode ) {
			var parts, part,
				lastIndex = 0;
				bbcode = CKEDITOR.bbcodetohtml(bbcode);
				this.onText( bbcode, 1 );
		}
	};
					...
					filters.push( function( element ) {
						...
						// Styled span could be either size or color.
							...
							if ( element.getStyle( 'text-align' ) )
								...
								name = 'dir';
							else if ( element.getAttribute( 'dir' ) )
								name = 'dir';
						} else if ( name == 'img' ) {
	...
	CKEDITOR.plugins.add( 'myimage', {
		...
		init: function( editor ) {
			...
			editor.ui.addButton( 'Image', {
				label: editor.lang.myimage.title,
				command: 'myimage',
				toolbar: 'blocks,20'
			});
			...
			CKEDITOR.dialog.add( 'myimageDialog', function ( editor ) {
				return {
					title: editor.lang.myimage.title,
					minWidth: 300,
					minHeight: 140,
					...
					contents: [
						{
							id: 'tab-basic',
							label: editor.lang.myimage.info,
							elements: [
								// UI elements of the first tab will be defined here
								{
									type: 'text',
									id: 'url',
									label: editor.lang.myimage.url
								}
							]
						},
						{
							id: 'adavance',
							label: editor.lang.myimage.adavance,
							elements: [
								// UI elements of the first tab will be defined here
								{
									type: 'text',
									id: 'width',
									label: editor.lang.myimage.width
								},
								{
									type: 'text',
									id: 'height',
									label: editor.lang.myimage.height
								}
							]
						}
					],
					onOk: function() {
						var dialog = this;
						var code = editor.document.createElement( 'img' );
						var src = dialog.getValueOf( 'tab-basic', 'url' );
						var w = dialog.getValueOf( 'adavance', 'width' );
						var h = dialog.getValueOf( 'adavance', 'height' );
						...
						h = h.replace('pt','');
						h = h.replace('%','');
						code.setAttribute('width', w);
						code.setAttribute('height', h);
	...
	CKEDITOR.plugins.add( 'code', {
		...
		init: function( editor ) {
			...
			editor.addCommand( 'phpblock', new CKEDITOR.dialogCommand( 'phpDialog' ));
			editor.ui.addButton( 'PhpBlock', {
				label: editor.lang.code.phptitle,
				command: 'phpblock',
				toolbar: 'blocks,20'
			});
			...
			CKEDITOR.dialog.add( 'codeDialog', function ( editor ) {
				return {
					...
					contents: [
						{
							id: 'tab-basic',
							label: editor.lang.code.add,
							elements: [
								// UI elements of the first tab will be defined here
								{
									type: 'html',
									html: '<p>'+editor.lang.code.entertext+'<br /><br /></p>'
								},					
								{
									type: 'textarea',
									id: 'texted',
					...
					onOk: function() {
						...
						var text = dialog.getValueOf( 'tab-basic', 'texted' );
						text = text.replace('<','&lt;');
						text = text.replace('>','&gt;');
						code.setHtml("<mybbir contenteditable=\"false\" style='border-bottom:1px solid #cccccc;width:100%;display:block;'>Code:</mybbir>"+text);
						code.setAttribute('class', 'code');
						editor.insertHtml('<br>');editor.insertElement( code );editor.insertHtml('<br>');
					}
				};
			});
			CKEDITOR.dialog.add( 'phpDialog', function ( editor ) {
				return {
					title: editor.lang.code.phptitle,
					minWidth: 500,
					minHeight: 140,
					...
					onOk: function() {
						...
						text = text.replace('<','&lt;');
						text = text.replace('>','&gt;');
						code.setHtml("<mybbir contenteditable=\"false\" style='border-bottom:1px solid #cccccc;width:100%;display:block;'>Php:</mybbir>"+text);
						editor.insertHtml('<br>');editor.insertElement( code );editor.insertHtml('<br>');
					}
				};
			});
   ...
   CKEDITOR.plugins.add("autosave", {
	...
	CKEDITOR.config.plugins = 'dialogui,dialog,about,a11yhelp,dialogadvtab,basicstyles,bidi,blockquote,clipboard,button,panelbutton,panel,floatpanel,colorbutton,colordialog,templates,menu,div,resize,toolbar,elementspath,enterkey,entities,popup,filebrowser,find,fakeobjects,flash,floatingspace,listblock,richcombo,font,forms,format,htmlwriter,horizontalrule,iframe,wysiwygarea,image,indent,indentblock,indentlist,smiley,justify,link,list,liststyle,magicline,maximize,newpage,pagebreak,pastetext,pastefromword,preview,print,removeformat,save,selectall,showblocks,showborders,sourcearea,specialchar,menubutton,scayt,stylescombo,tab,table,tabletools,undo,wsc,bbcode,customstylesheet,myimage,code,videos,confighelper,divarea';
