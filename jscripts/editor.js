...
/*
	...
	initialize: function(textarea, options)
	{
		...
		$$('script').each(function(script) {
			...
			if(script.src && script.src.indexOf('ckeditor.js') != -1) {
				ckeditorjs = 1;
			}
		}, this);
		if(ckeditorjs == 0)
		{
			if ( document.head && (!document.readyState || document.readyState == 'complete') )
			{
				var script = document.createElement( 'script' );
				script.type = 'text/javascript';
				script.src = this.baseURL.replace('/jscripts','') + 'ckeditor/ckeditor.js';
				document.head.appendChild( script );
			}
			else
			{
				document.write(
					'<script type="text/javascript" src="' + this.baseURL.replace('/jscripts','') + 'ckeditor/ckeditor.js' + '"></script>' );
			}
		}
...
return true;
	...
	performInsert: function(open_tag, close_tag, is_single, ignore_selection)
	{
		...
		eval('editor = CKEDITOR.instances.'+textarea+';');
		editor.insertHtml(open_tag);
		return is_single;
	},
	...
	insertSmilie: function(e)
	{
		...
		this.performInsert('<img src="'+element.src+'" class="smilie" alt="'+element.alt+'" />', "", true, false);
	},
